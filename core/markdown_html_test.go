...
func TestNeedsRichMessage(t *testing.T) {
	cases := []struct {
		...
		// L-0691: list runs no longer trigger RichMessage — only tables do.
		// A message with bullets/numbered items takes the lower-latency HTML path.
		{"single list item uses HTML", "- one", false},
		{"two unordered list items are not a table", "- one\n- two", false},
		{"two ordered list items are not a table", "1. one\n2. two", false},
		{"three unordered list items are not a table", "- one\n- two\n- three", false},
		{"three ordered list items are not a table", "1. one\n2. two\n3. three", false},
		{"mixed list items are not a table", "- one\n2. two\n- three", false},
		{"list run with a table still triggers on the table", "- one\n- two\n\n| A | B |\n| --- | --- |\n| 1 | 2 |", true},
