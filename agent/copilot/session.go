...
func (cs *copilotSession) Send(prompt string, images []core.ImageAttachment, files []core.FileAttachment) error {
	...
	if cs.identityInjected.CompareAndSwap(false, true) {
		...
		var project, sessionKey, ccBin, ccConfig, ccDataDir, ccPersonasDir, personaClass, relayTarget, rehydrationDigest, archiveDir string
		...
		if project != "" && !cs.hasAgentArg {
			...
			if personaClass != "" {
				prompt += "\n\n" + core.ComposePersona(ccPersonasDir, core.PersonaClass(personaClass), rawPersona, archiveDir) + "\n"
			} else if rawPersona != "" {
				prompt += "\n\n" + rawPersona + "\n"
			}
